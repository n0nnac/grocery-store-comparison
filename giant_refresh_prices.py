...
CATEGORY_NEGATIVES = {
    ...
    "produce": {"oil", "frozen", "ice", "cream", "yogurt", "snack", "candy", "chocolate", "wine", "beer", "roasted", "pickled", "jarred"},
    "pantry": {"frozen", "ice", "cream", "wine", "beer", "spirits", "ravioli", "ready"},
...
def candidate_score(meal_key, meal_record, product):
    ...
    if (product.get("brand") or "").lower() == "giant":
        score += 0.05
...
def refresh(args):
    ...
    for meal_key, item in candidates:
        ...
        saved_pid = giant_source(item).get("product_id")
